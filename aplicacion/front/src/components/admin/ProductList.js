...
const ProductsList = () => {
    ...
    const deleteProductHandler= (id)=> { //funcion que se ejecuta cuando se envia el formulario
        const response=window.confirm("Esta seguro de querer borrar este producto?")
        if (response){ //si la respuesta es true
            dispatch(deleteProduct(id))
            alert("Producto eliminado correctamente")
            window.location.reload(false) //recarga la pagina con los datos actualizados
        }
    }
